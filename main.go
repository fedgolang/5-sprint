...
func isDurationZero(duration time.Duration) bool {
	return duration == 0
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	spentCalories := ((CaloriesMeanSpeedMultiplier * speed * CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
...
func (w Walking) Calories() float64 {
	...
	height := w.Height * CmInM // Переводим рост в метры
	spentCalories := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return spentCalories
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	speed := s.meanSpeed()
	spentCalories := (speed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return spentCalories
}
